...
class ProtoGCN(nn.Module):
    ...
    def __init__(self,
        ...
        # Propagate the graph's joint count to every TCN block so that
        # mstcn.add_coeff has the correct size for non-NTU skeletons (e.g. G1).
        num_joints = int(A.size(1))
        for kw in lw_kwargs:
            kw.setdefault('tcn_num_joints', num_joints)
