"""Convert G1 CSV motion data into the pickle format consumed by the
...
_ROOT = _HERE.parent                             # release / dataset root
_DATA = _HERE / 'data' / 'pkl'                  # Action_Recognition/data/pkl/

# Maps variant name -> subdirectory under the dataset root.
# The GRL baseline (g1_sanitized_100) was produced by a separate GRL
# sanitizer not included in this release; its entry is simply skipped
# when the directory is absent.
VARIANT_ROOTS: Dict[str, Optional[str]] = {
    'original':    None,                        # the dataset root itself
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description='Convert G1 CSV data to the action-classifier pickle format',
