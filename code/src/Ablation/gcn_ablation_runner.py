#!/usr/bin/env python3
"""
UNVEIL DS-GCN Ablation Experiment Runner (2026)
=======================================================
...
Usage:
    python Ablation/gcn_ablation_runner.py --task all --ablation all \
        --data-root . --splits-dir artifacts/splits

Re-run a single pair without touching others:
    python Ablation/gcn_ablation_runner.py --task gender --ablation fc_graph
"""
...
for _p in [str(_BONES_ROOT), str(_BONES_ROOT / "src"), str(_ABLATION_DIR)]:   # src/ holds pyskl + DS-GCN
    if _p not in sys.path:
        sys.path.insert(0, _p)

# ---------------------------------------------------------------------------
# Import from the reference implementation.
# The unveil import triggers the pyskl path-setup side-effects, making
# DGSTGCN and the real pyskl Graph available in sys.modules.
# ---------------------------------------------------------------------------
...
# Reuse actor-subsampling and position-only wrapper from the UNVEIL runner
from ablation_runner import subsample_actors, PositionOnlyDataset  # noqa: E402

from configs_gcn import (  # noqa: E402
    ALL_ABLATIONS_DSGCN,
...
# ---------------------------------------------------------------------------
# The unveil import loads the real pyskl Graph via the stub and registers
...
def _apply_no_temporal(model: DynGraphBackbone) -> DynGraphBackbone:
...
class DynGraph_NoTemporal(DynGraphBackbone):
    """DynGraphBackbone with all temporal convolutions replaced by identity."""
...
class DynGraph_FCGraph(DynGraphBackbone):
    """DynGraphBackbone with fully-connected G1 graph (no kinematic bias)."""
    ...
    def __init__(self, fmt, num_classes, emb_dim=256, base_channels=64,
        ...
        self._reshape_input = types.MethodType(
            DynGraphBackbone._reshape_input, self
        )
        self.forward = types.MethodType(DynGraphBackbone.forward, self)
...
class _TriOutput(nn.Module):
    """Adapts the backbone's (logits, z) output to the (logits, z, aux)
    interface expected by the shared eval helpers in unveil.py."""
...
def build_model_dsgcn(
    ...
    if ablation_cfg["no_temporal"]:
        model = DynGraph_NoTemporal(**kwargs)
    elif ablation_cfg["graph_layout"] == "bones_seed_g1_fc":
        model = DynGraph_FCGraph(**kwargs)
    else:
        model = DynGraphBackbone(**kwargs)
...
def run_ablation_dsgcn(
    ...
    model_eval = _TriOutput(model)   # (logits, z, aux) view for the eval helpers
    ...
    for epoch in range(1, epochs + 1):
        ...
        if epoch % eval_every == 0 or epoch == epochs:
            if task == "reid":
                ...
                su_acc, su_c, su_n = eval_cls(model_eval, seen_unseen_loader)
                metric_str = f"seen-unseen={su_acc:.4f} ({su_c}/{su_n})"
                if unseen_actors_loader is not None:
                    un_acc, un_c, un_n = eval_cls(model_eval, unseen_actors_loader)
    ...
    eval_model.eval()
    final_eval = _TriOutput(eval_model)
    ...
    if task == "reid":
        ...
        su_acc, _, _ = eval_cls(final_eval, seen_unseen_loader)
        result["seen_actors_unseen_demos_accuracy"] = su_acc
        log.info(f"  seen-actors-unseen-demos acc={su_acc:.4f}")

        if unseen_actors_loader is not None:
            un_acc, _, _ = eval_cls(final_eval, unseen_actors_loader)
...
# ---------------------------------------------------------------------------
# Summary table (shared display logic with the UNVEIL runner)
# ---------------------------------------------------------------------------
...
def print_summary(
    ...
    from ablation_runner import print_summary as _base_print_summary
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="UNVEIL DS-GCN Ablation Runner")
...
def main():
    ...
    if args.device:
        ...
        globals()["DEVICE"] = args.device  # rebind this module's copy too
