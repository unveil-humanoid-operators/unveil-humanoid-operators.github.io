"""Ablation experiment configurations for UNVEIL (2026).
...
Each variant degrades a DIFFERENT pipeline stage:
  ...
  pos_only_v2   — input stage: encoder reduced to a single stream (pos only)
  subset_fc     — spatial stage: 20/35 channels (root + legs + waist yaw/roll)
                  grouped into 9 joints + a single fully-connected graph
...
# A0: Full UNVEIL — nothing degraded
ABLATION_FULL: Dict[str, Any] = dict(
    ablation="full",
    description="Full UNVEIL (baseline — all components enabled)",
    model_class="default",
