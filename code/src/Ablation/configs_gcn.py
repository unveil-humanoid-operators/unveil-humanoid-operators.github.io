"""Ablation experiment configurations for the DS-GCN comparison backbone (2026).

Same 4 ablation variants as the UNVEIL runner but with DS-GCN-specific
hyperparameters:
  - base_channels=64  (vs 96 for UNVEIL)
  - supcon_warmup=20  (vs 8 for UNVEIL)
  - epochs=10         (vs 50 for UNVEIL); note supcon_warmup=20 > epochs=10,
                      so SupCon never activates in this configuration
  - no num_prototype / lambda_csc (DS-GCN has no signature bank)
"""
...
# ---------------------------------------------------------------------------
# Ablation variants (same 4 as the UNVEIL runner — only graph_layout differs for A2)
# ---------------------------------------------------------------------------
