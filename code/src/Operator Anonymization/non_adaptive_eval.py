...
Uses the PRE-TRAINED UNVEIL checkpoints (trained on original G1 data)
...
The GRL baseline was produced by a separate GRL sanitizer not included in
this release; its entry is skipped when the directory is absent.

Results saved to: non_adaptive_eval_results.json (next to this script)

Usage (from the release root):
  python "Operator Anonymization/non_adaptive_eval.py"
"""
...
UNVEIL_CKPT_BASE = str(_PROJECT_ROOT / "artifacts" / "models" / "unveil" / "unveil" / "actor_holdout_split_g1")
...
DATASETS = {
    "Original G1 (no defense)": {
        ...
        "result_json_dir": str(_PROJECT_ROOT / "artifacts" / "models" / "unveil" / "unveil" / "actor_holdout_split_g1"),
...
def load_unveil(task: str) -> Tuple[UnveilBackbone, dict]:
    ckpt_path = os.path.join(UNVEIL_CKPT_BASE, task, "best_model.pt")
    ckpt = torch.load(ckpt_path, map_location=DEVICE, weights_only=False)
    cfg  = ckpt["config"]
    model = UnveilBackbone(
        fmt=FMT,
...
def collect_regression_preds(
    ...
    for xb, lengths, yb, _ in loader:
        xb = xb.to(DEVICE)
        logits, _, _ = model(xb)   # UNVEIL returns (logits, z, signature)
...
def eval_dataset(name: str, cfg: dict, active_tasks: List[str]) -> dict:
    """Run inference with pre-trained UNVEIL on one sanitized dataset."""
    ...
    for task in active_tasks:
        ...
        model, model_cfg = load_unveil(task)
...
def main():
    import argparse
    p = argparse.ArgumentParser(description="Non-adaptive UNVEIL evaluation on sanitized G1 data")
