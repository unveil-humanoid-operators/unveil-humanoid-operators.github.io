...
Output CSVs mirror the original g1/csv/ layout so the sanitized dataset can
be evaluated directly with "Operator Anonymization/non_adaptive_eval.py".

Also exposes G1PMRSanitizerWrapper — a sanitize_clip() interface so MSE can
be computed on-the-fly.

Usage (from the release root):
  python "Operator Anonymization/apply_g1_pmr.py" --checkpoint g1_pmr_ckpts/final.pt --data-root .
  python "Operator Anonymization/apply_g1_pmr.py" --checkpoint g1_pmr_ckpts/final.pt --data-root . --num-dummies 5
"""
...
sys.path.insert(0, str(_HERE))                    # models/ + dataset_g1_paired.py

from models.pmr import PMRModel
from dataset_g1_paired import _load_clip, _normalise, G1_CHANNELS, T_WINDOW
from unveil import load_g1_cache, g1_cache_fetch, read_g1_motion
...
class G1PMRSanitizerWrapper:
    ...
    Implements sanitize(x) for on-the-fly clip sanitization.
    """
...
def sanitize_clip_pmr(
    ...
    """Sanitize one clip: PMR motion factor + dummy privacy factor."""
    T = x.shape[0]
...
def main():
    ...
    print('To evaluate: python "Operator Anonymization/non_adaptive_eval.py" '
          f"(sanitized dataset at {output_root})")
